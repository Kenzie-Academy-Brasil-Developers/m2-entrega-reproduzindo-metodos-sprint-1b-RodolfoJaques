...
newForEach(arrayTestForEach, callbackForEach);console.assert(typeof newForEach === "function","Sua Função newForEach ou não está declarada ou está com nome incorreto");
...
function newMap(arr,callback){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        novoArr.push(callback(arr[i],i,arr));
    }
    return novoArr;
}
...
function newFilter(arr,callback){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        let resultado = callback(arr[i],i,arr);
...
function newIndexOf(array, elementoDePesquisa, inicio = 0){
    ...
    if(inicio >= array.length){
        ...
        const finalLoop = array.length + inicio;

        for(let i = array.length - 1; i >= finalLoop; i--){
...
function newIncludes(array,elementoDePesquisa,inicio = 0){
...
function newSlice(array,inicio = 0,fim = array.length){
    ...
    if(inicio >= 0){
        ...
        const initialLoop = array.length + inicio;

        for(let i = initialLoop; i <= array.length - 1; i++){
...
function newFlatMap(arr,callback){
    ...
    for(let i = 0; i < novoArr.length; i++){
        ...
        if(typeof novoArr[i] === 'array'){

            for(let j = 0; j < novoArr[i].length; j++){

                arrFinal.push(novoArr[i][j][0]);
            }
        }else{

            arrFinal.push(novoArr[i][0]);
        }
    }
...
//console.log(newFlatMap(newFlatMapTest,callbackNewFlatMap1));
//console.log(newFlatMap(newFlatMapTest,callbackNewFlatMap2));

//let x = newFlatMapTest.flatMap(callbackNewFlatMap1);
//console.log(x);
//let y = newFlatMapTest.flatMap(callbackNewFlatMap2);
//console.log(y);
